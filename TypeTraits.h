...
namespace stdlib
{
	...
	template< typename T >
	struct add_pointer { using type = conditional_t<is_function<T>::value, T, remove_reference_t<T>*>; };

	template< typename T, typename ... Args >
	struct add_pointer<T(Args...)> { using type = T(*)(Args ...); };
	...
	template< typename T >
	struct make_signed<const T> { using type = const make_signed<T>; };

	template< typename T >
	struct make_signed<volatile T> { using type = volatile make_signed<T>; };

	template< typename T >
	struct make_signed<const volatile T> { using type = const volatile make_signed<T>; };

	template<>
	struct make_signed<char> { using type = signed char; };

	template<>
	struct make_signed<signed char> { using type = signed char; };

	template<>
	struct make_signed<unsigned char> { using type = signed char; };

	template<>
	struct make_signed<signed short> { using type = signed short; };

	template<>
	struct make_signed<unsigned short> { using type = signed short; };

	template<>
	struct make_signed<signed int> { using type = signed int; };

	template<>
	struct make_signed<unsigned int> { using type = signed int; };

	template<>
	struct make_signed<signed long int> { using type = signed long int; };

	template<>
	struct make_signed<unsigned long int> { using type = signed long int; };

	template<>
	struct make_signed<signed long long int> { using type = signed long long int; };

	template<>
	struct make_signed<unsigned long long int> { using type = signed long long int; };
	...
	template< typename T >
	struct make_unsigned<const T> { using type = const make_unsigned<T>; };

	template< typename T >
	struct make_unsigned<volatile T> { using type = volatile make_unsigned<T>; };

	template< typename T >
	struct make_unsigned<const volatile T> { using type = const volatile make_unsigned<T>; };
	...
	template<>
	struct make_unsigned<signed long int> { using type = unsigned long int; };

	template<>
	struct make_unsigned<unsigned long int> { using type = unsigned long int; };

	template<>
	struct make_unsigned<signed long long int> { using type = unsigned long long int; };

	template<>
	struct make_unsigned<unsigned long long int> { using type = unsigned long long int; };
	...
	template< typename T >
	struct is_null_pointer : is_same<T, remove_cv_t<decltype(nullptr)>> {};
	...
	template< typename T >
	struct rank : integral_constant<decltype(sizeof(0)), 0> {};

	template< typename T >
	struct rank<T[]> : integral_constant<decltype(sizeof(0)), rank<T>::value + 1> {};

	template< typename T, decltype(sizeof(0)) N >
	struct rank<T[N]> : integral_constant<decltype(sizeof(0)), rank<T>::value + 1> {};
	...
	struct extent<T[I], 0>: integral_constant<decltype(sizeof(0)), 0> {};
	...
	template< typename T, typename U >
	struct common_type<T, U> : conditional_t<(is_same<T, decay_t<T>>::value && is_same<U, decay_t<U>>::value), enable_if<(is_void<void_t<decltype(false ? declval<T>() : declval<U>())>>::value), decay_t<decltype(false ? declval<T>() : declval<U>())> >, common_type<decay_t<T>, decay_t<U>>> {};
	template< typename T, typename U, typename ... Ts >
	struct common_type<T, U, Ts...> : conditional_t<(is_void<void_t<common_type_t<T, U>>>::value), common_type<common_type_t<T, U>, Ts ...>, common_type<>> {};
	...
	template<>
	struct is_integral<signed long int> : true_type {};

	template<>
	struct is_integral<unsigned long int> : true_type {};

	template<>
	struct is_integral<signed long long int> : true_type {};
	...
	template< typename T >
	struct is_signed : conditional_t<is_arithmetic<T>::value, bool_constant<(T(-1) < T(0))>, false_type> {};


	template< typename T >
	struct is_unsigned : conditional_t<is_arithmetic<T>::value, bool_constant<(T(-1) > T(0))>, false_type> {};
	...
	template< typename T >
	struct is_member_function_pointer<const T> : is_member_function_pointer<T> {};

	template< typename T >
	struct is_member_function_pointer<volatile T> : is_member_function_pointer<T> {};

	template< typename T >
	struct is_member_function_pointer<const volatile T> : is_member_function_pointer<T> {};
